...
$getResource = Get-AzResource -Name $dataCollectionRuleName -ResourceGroupName $resourceGroupName 
...
try{
    ...
    New-AzResourceGroupDeployment -ResourceGroupName $resourceGroupName -TemplateFile "./LogicApp.json" -workflowName $LogicAppName -location $location -tenantId $tenantId -clientId $sp.AppId -clientSecret $secureSpSecret -immutableId $immutableId -tableName $tableName
